<template lang="">
  <div
    ...
    <div id="delete-warning"
      class="hidden p-2 z-50 shadow rounded-lg absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-primary text-secondary">
      <h2 class="text-center font-semibold text-red-500 text-3xl">Warning</h2>
      <p class=" text-lg font-medium text-center">Are you sure you want to delete this blog and all its history?</p>
    ...
    <router-link :to="{name: 'ViewBlog', params: {blogid: this.post.blogID}}" class=''>
      ...
      <div class="w-full flexCenter ">
        ...
        <div class="relative rounded-full flexCenter overflow-hidden w-16 h-16 m-2 mr-0 ">
          <img v-show="post.profilePicture" class="w-full rounded-full" :src="post.profilePicture"
  ...
  export default {
    ...
    methods: {
      ...
      showWarning() {
        ...
        if (delete_warning.classList.contains('hidden')) {
          delete_warning.classList.remove('hidden');
          return;
        }
        delete_warning.classList.add('hidden');
        return;
      }
    },
  };
